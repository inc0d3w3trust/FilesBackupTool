	...
	public List<RestoreEntryModel> getBackupTargetList(String timeStamp) throws Exception {
		...
		if(backupSourceList.size() > 0) {
			...
			for(String backupSource : backupSourceList) {
				...
				useDeleteBackupSource = applicationSettings.getSourceFolderDeleteActionAfterBackupByPath(backupSource);
				
				if(useArchive == -1) {
					useArchive = 0;
				}
				
				if(useDeleteBackupSource == -1) {
					useDeleteBackupSource = 0;
				}
				
				boolean booleanUseArchive = (useArchive > 0) ? true : false;
				boolean booleanUseDeleteBackupSource = (useDeleteBackupSource > 0) ? true : false;
				
				RestoreEntryModel restoreEntryModel = new RestoreEntryModel(backupSource, nodeName, booleanUseArchive);
				restoreEntryModel.setisDeleteSource(booleanUseDeleteBackupSource);
